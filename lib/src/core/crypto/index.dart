...
List<Felt> functionCallsToCalldataLegacy({required List<FunctionCall> functionCalls}) {
  List<Felt> calldata = [];
  List<Felt> calls = [];
  for (final call in functionCalls) {
    calls.addAll([
  ...
  return [Felt.fromInt(functionCalls.length), ...calls, Felt.fromInt(calldata.length), ...calldata];
}
